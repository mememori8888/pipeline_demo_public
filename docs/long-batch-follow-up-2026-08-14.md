...
The title is generated by Gemini in Japanese after the final integration step, using the integrated transcription content and source file names. The same title is used for the first top-level Markdown heading and the Drive file name. If title generation fails, the fallback name is `integrated_book_<job_id>.md`.
